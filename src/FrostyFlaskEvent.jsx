...
const EVENT = {
  id: "feb2027",
  name: "Frosty Flask Ruck",
  route: "Sandwich → Deal → Sandwich",
  distanceMiles: 12.5,
  dateStr: "13 February 2027",
  meetPoint: "Sandwich Quayside Car Park",
  meetTime: "1:00 PM",
  checkpoints: [
    { id: "cp1", name: "The Barbican, Sandwich", desc: "First stamp — the old toll bridge gate by the quay.", instructions: "Get the crew together at the Barbican before heading out of town.", emoji: "🎒" },
    { id: "cp2", name: "Royal St George's Golf Club", desc: "Open Championship links course on the coast path.", instructions: "Quick group photo by the clubhouse, then push on toward Deal.", emoji: "⛳" },
    { id: "cp3", name: "Deal Pier", desc: "Deal's iconic pier and the turnaround point.", instructions: "Get the whole group at the pier entrance. Quick photo, then turn for home.", emoji: "🎣" },
    { id: "cp4", name: "Sandwich Quayside", desc: "Final stamp — proof you made it back.", instructions: "Take this photo back at Sandwich Quayside to confirm you're home.", emoji: "🏁" },
  ],
};

const META_KEY = "ruckbuzz-feb2027:participant";
...
function LoopGraphic() {
  return (
    <svg viewBox="0 0 360 76" width="100%" role="img" aria-label="Sandwich to Deal, 12.5 mile loop" style={{ display: "block", maxWidth: "100%" }}>
      <text x="28" y="45" fill={CREAM} fontSize="13" fontWeight="700">SANDWICH</text>
      <circle cx="112" cy="38" r="8" fill={LIME} />
      <text x="180" y="33" fill="white" fontSize="22" fontWeight="800" textAnchor="middle">12.5 MI</text>
      <text x="180" y="54" fill="#BDBDB8" fontSize="11" fontWeight="700" textAnchor="middle">LOOP</text>
      <circle cx="248" cy="38" r="8" fill={LIME} />
      <text x="270" y="45" fill={CREAM} fontSize="13" fontWeight="700">DEAL</text>
    </svg>
  );
}

export default function FrostyFlaskEvent({ onExit, ruck500Name = "" }) {
  ...
  async function resetEvent() {
    const ok = confirm(
      "Reset this Buzz Ruck? This removes your February Buzz Ruck progress, checkpoint photos, start/finish times and certificate from this device. Your main Ruck 500 log will not be affected."
    );
    if (!ok) return;
    ...
    setNow(Date.now());
    setScreen("landing");
    showToast("February Buzz Ruck reset.");
  }
  ...
  const topbar = (
    <div style={{ position: "sticky", top: 0, zIndex: 5, background: "rgba(13,13,13,0.96)", borderBottom: "1px solid #262626" }} className="px-5 py-3 flex items-center justify-between">
      ...
      <span className="font-bold uppercase tracking-wide text-xs" style={{ color: "#c8c8c2" }}>Frosty Flask Ruck</span>
    </div>
  );
      ...
      <div style={shell}>
        ...
        <section style={{ background: LIME, color: INK }} className="px-5 pt-8 pb-7">
          <div className="max-w-md mx-auto text-center">
            ...
            <h1 className="display" style={{ fontSize: "3.5rem", lineHeight: 0.9 }}>FROSTY FLASK<br />BUZZ RUCK</h1>
            <p className="font-bold mt-4">13 February 2027 · Sandwich</p>
            <div className="mt-5 rounded-2xl overflow-hidden" style={{ border: `2px solid ${INK}` }}>
              <img src="/frosty-flask-event.jpg" alt="Frosty Flask Ruck route" style={{ width: "100%", height: 150, objectFit: "cover", display: "block" }} />
            </div>
      ...
      <div style={shell}>
        ...
        <div className="min-h-[calc(100dvh-52px)] flex items-center">
          <div className="max-w-md mx-auto w-full px-5 py-8 text-center">
            ...
            <p className="mt-5 text-sm opacity-70">Start when you're ready at Sandwich Quayside Car Park.</p>
